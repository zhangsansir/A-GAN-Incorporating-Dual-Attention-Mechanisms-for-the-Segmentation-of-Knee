...
#xrange在python3.*以后的版本已经不用了，直接用range，不需要另外引用。
#from six.moves import xrange

from ops import conv2d,lrelu,conv2d_,deconv2d_,conv2d_1,deconv2d_my,dice_loss_weight,BN,se_block
from utils import load_data,load_image,save_images,imread,merge,imsave,save_mat

class se_skip_gate(object):
    def __init__(self, sess, image_size=384,
                 batch_size=2, sample_size=1, output_size=256,
                 gf_dim=32, df_dim=32, L1_lambda=20,
                 input_c_dim=1, output_c_dim=5, dataset_name='facades',
...
#                                        name='real_A_and_B_images')
        ...
        #real_B:手动标记，real_A:MRI图像，fake_B：G(x)，real_AB：x|y，fake_AB：G(x)|y
        ...
        self.fake_B,self.fake_B_48,self.fake_B_96,self.fake_B_192 = self.generator(self.real_A,is_training=self.is_training,dropout_rate=self.dropout_rate)
        #D_logits：D(x|y)，D_logits_：D(G(x)|y)
        self.real_AB = tf.concat([self.real_A, self.real_B], 3)
        self.fake_AB = tf.concat([self.real_A, self.fake_B], 3)
        self.real_A_192,self.real_A_96,self.real_A_48=self.downsample_module(self.real_A,is_training=self.is_training)
        self.D, self.D_logits = self.discriminator(self.real_AB,self.real_A_192,self.real_A_96,self.real_A_48,is_training=self.is_training ,reuse=False)
        self.D_, self.D_logits_ = self.discriminator(self.fake_AB,self.fake_B_192,self.fake_B_96,self.fake_B_48, is_training=self.is_training,reuse=True)

        self.fake_B_sample,self.fake_B_sample_48,self.fake_B_sample_96,self.fake_B_sample_192 = self.sampler(self.real_A,is_training=self.is_training,dropout_rate=self.dropout_rate)

        self.d_sum = tf.summary.histogram("d", self.D)
        self.d__sum = tf.summary.histogram("d_", self.D_)
        self.fake_B_sum = tf.summary.image("fake_B", self.fake_B)

        #tf.reduce_mean不指定维度时默认求所有元素的均值
...
#                        + self.L1_lambda * tf.reduce_mean(tf.abs(self.real_B - self.fake_B))
        ...
        # self.g_loss2 = self.L1_lambda * tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=self.fake_B, labels=self.real_B))+ self.L1_lambda * tf.reduce_mean(tf.abs(self.real_B - self.fake_B))
        
        # class_weights = tf.constant([1.0, 1.0, 5.0, 5.0, 0.1])
        # class_weights = tf.constant([1.0, 1.0, 10.0, 9.0, 0.1])
        # class_weights = tf.constant([1.0, 1.0, 20.0, 18.0, 0.1])
        class_weights = tf.constant([1.0, 1.0, 15.0, 12.0, 0.1])
        # class_weights = tf.constant([1.0, 1.0, 13.0, 11.0, 0.1])
...
#        self.g_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.D_logits_, labels=tf.ones_like(self.D_))) 
    ...
    def load_random_samples(self):
        data_=[]
        for root,dirs,files in os.walk('./datasets/{}/test'.format(self.dataset_name),topdown='False'):
            for dir in dirs:
    ...
    def load_train_samples(self):
        sample_data = glob('./datasets/{}/trainsample/*.mat'.format(self.dataset_name))
    ...
    # 设置x轴和y轴的刻度为1并显示每一个刻度
        ax.xaxis.set_major_locator(ticker.MultipleLocator(1))  # x轴刻度间隔为1
        ax.yaxis.set_major_locator(ticker.MultipleLocator(1))  # y轴刻度间隔为1
        ...
        with open("loss.txt", 'w') as f:
            for i, (d_loss, g_loss) in enumerate(loss):
                f.write(f"Epoch {i + 1}: Discriminator Loss = {d_loss}, Generator Loss = {g_loss}\n")
...
#        self.writer = tf.summary.FileWriter("./logs", self.sess.graph)
        ...
        data=[]
        print('2')
        for root,dirs,files in os.walk('./datasets/{}/train'.format(self.dataset_name),topdown='False'):
            for dir in dirs:
                dirs_path=os.path.join(root,dir)
                imgs=os.listdir(dirs_path)
                for img in imgs:
        ...
        validation_data=glob('./datasets/{}/val/*.mat'.format(self.dataset_name))
        print('4')
        for epoch in range(args.epoch):
            print(epoch)
            # *：匹配0个或多个字符；?：匹配单个字符；[]：匹配指定范围内的字符，如：[a-z]匹配所有字母
...
#                 errG_val = self.g_loss.eval({self.real_data: validation_images})
                ...
                if np.mod(counter,batch_idxs+1) ==1:
    ...
    def   downsample_module(self,image,is_training=True,y=None, reuse=False):
        with tf.variable_scope("downsample_module") as scope:


            image=tf.reshape(image, tf.stack([-1, self.image_size,self.image_size, self.input_c_dim ]))
            # image is (384 x 384 x input_c_dim)
            r1_ = conv2d_(image, self.gf_dim, name='g_r1_conv_')
            # e1_ is (384 x 384 x self.gf_dim)
            
            r1 = BN(conv2d(r1_, self.gf_dim, name='g_r1_conv'),name='g_bn_r1',is_training=is_training)
            # e1 is (192 x 192 x self.gf_dim)
            r2_ = BN(conv2d_(lrelu(r1), self.gf_dim*2, name='g_r2_conv_'),name='g_bn_r2_',is_training=is_training)
            # e2_ is (192 x 192 x self.gf_dim*2)
            self.r2_out = conv2d_(r2_,1,k_h=1,k_w=1,name='g_r2_out_conv')
            
            r2 = BN(conv2d(lrelu(r2_), self.gf_dim*2, name='g_r2_conv'),name='g_bn_r2',is_training=is_training)
            # e2 is (96 x 96 x self.gf_dim*2)
            r3_ = BN(conv2d_(lrelu(r2), self.gf_dim*4, name='g_r3_conv_'),name='g_bn_r3_',is_training=is_training)
            # e3_ is (96 x 96 x self.gf_dim*4)
            self.r3_out = conv2d_(r3_,1,k_h=1,k_w=1,name='g_r3_out_conv')
            r3 = BN(conv2d(lrelu(r3_), self.gf_dim*4, name='g_r3_conv'),name='g_bn_r3',is_training=is_training)
            # e3 is (48 x 48 x self.gf_dim*4)
            r4_ = BN(conv2d_(lrelu(r3), self.gf_dim*8, name='g_r4_conv_'),name='g_bn_r4_',is_training=is_training)
            # e4_ is (48 x 48 x self.gf_dim*8)
            self.r4_out = conv2d_(r4_,1,k_h=1,k_w=1,name='g_r4_out_conv')
            return self.r2_out,self.r3_out,self.r4_out



    def   discriminator(self, image, image192,image96,image48, is_training=True, y=None, reuse=False):
        ...
        with tf.variable_scope("discriminator") as scope:
            ...
            h0__=tf.concat([h0,image192],axis=3)
            print(h0__.shape)
            h1 = lrelu(BN(conv2d(h0__, self.df_dim*2, name='d_h1_conv'),name='d_bn1',is_training=is_training))
            # h1 is (96 x 96 x self.df_dim*2)
            # h1_=lrelu(conv2d_(image96,self.df_dim*2,k_h=1,k_w=1,name='d_h1__conv'))
            h1__=tf.concat([h1,image96],axis=3)
            print("h1__ shape:",h1__.shape)
            h2 = lrelu(BN(conv2d(h1__, self.df_dim*4, name='d_h2_conv'),name='d_bn2',is_training=is_training))
            # h2 is (48 x 48 x self.df_dim*4)
            # h2_=lrelu(conv2d_(image48,self.df_dim*4,k_h=1,k_w=1,name='d_h2__conv'))
            h2__=tf.concat([h2,image48],axis=3)
            print("h2__ shape:",h2__.shape)
            h3 = lrelu(BN(conv2d(h2__, self.df_dim*8,  name='d_h3_conv'),name='d_bn3',is_training=is_training))
...
#            print('aaaaaa')
            ...
            # e5_se= se_block(e5_, 'se_block_e5')
            # e5_ = e5_+e5_se
            ...
            # e4_=conv2d_1(lrelu(e4_), self.gf_dim*4, name='d_e4_conv')
            ...
            self.d2_1 = se_block(self.d2_1, 'se_block_d2_1')
            ...
            # e3_=conv2d_1(lrelu(e3_), self.gf_dim*2, name='d_e3_conv')
            ...
            self.d3_1 = se_block(self.d3_1, 'se_block_d3_1')
            ...
            # e2_=conv2d_1(lrelu(e2_), self.gf_dim, name='d_e2_conv')
            ...
            self.d4_1 = se_block(self.d4_1, 'se_block_d4_1')
            ...
            # e1_=conv2d_1(lrelu(e1_), self.gf_dim, name='d_e1_conv')
            ...
            self.d5_out=conv2d_(self.d5_1,1,k_h=1,k_w=1,name='g_d5_out_conv')
            self.d5_1 = se_block(self.d5_1, 'se_block_d5_1')

            
            self.d6 = conv2d_(lrelu(self.d5_1), self.output_c_dim, name='g_d6_0_conv_')

            return tf.nn.tanh(self.d6),self.d2_out,self.d3_out,self.d4_out
    ...
    def save(self, checkpoint_dir, step):
        ...
        model_dir = "%s_%s_%s_msg_cgm改_lr0.001" % (self.dataset_name, self.batch_size, self.output_size)
    ...
    def load(self, checkpoint_dir):
        ...
        #如果要指定加载某个epoch，用这段代码
        # model_dir = "%s_%s_%s_msg_cgm改_lr0.001" % (self.dataset_name, self.batch_size, self.output_size)
        # checkpoint_dir = os.path.join(checkpoint_dir, model_dir)
        # ckpt_name='pix2pix.model-19'
        # print(os.path.join(checkpoint_dir, ckpt_name))
        # self.saver.restore(self.sess, os.path.join(checkpoint_dir, ckpt_name))
    ...
    def test(self,args):
        data = glob('./datasets/{}/test/*.mat'.format(self.dataset_name))
        #np.random.shuffle(data)
...
#                        './{}/testTruth_{:d}.bmp'.format(args.test_dir, idx))
    def  finaltest(self,args):
        ...
        for root,dirs,files in os.walk('./datasets/{}/each_test'.format(self.dataset_name)):
            for dir in dirs:
...
#                       print(img_AB.shape)
    ...
    def  finaltest3(self,args):
        ...
        for root,dirs,files in os.walk('./datasets/{}/test'.format(self.dataset_name)):
            for dir in dirs:
                dir_path=os.path.join(root,dir)
                imgs=os.listdir(dir_path)
                imgs.sort(key=lambda x:int(x[:-4]))
        ...
        for root,dirs,files in os.walk('./datasets/{}/each_test'.format(self.dataset_name)):
            for dir in dirs:
                ...
                for img in imgs:
                    ...
                    shape = image.shape
                    image = image.reshape((1,shape[0],shape[1],shape[2]))
                    image2=load_data('./datasets/{}/each_test/changyuan/6.mat'.format(self.dataset_name))
                    image3=load_data('./datasets/{}/each_test/caofenglian/12.mat'.format(self.dataset_name))
                    image4=load_data('./datasets/{}/each_test/gaosi/8.mat'.format(self.dataset_name))
